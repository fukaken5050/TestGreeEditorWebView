...
public class TestMyWebViewEditorWindow : MyWebViewEditorWindow
{
	public object m_GlobalObject = (object) null;

	[MenuItem ("Test/MyWebView")]
	static void Create()
	{
		var win = TestMyWebViewEditorWindow.Create<TestMyWebViewEditorWindow>( "TestMyWebView", null, 200, 530, 800, 600 );
	//	win.AddGlobalObject( Type.GetType( win.m_GlobalObjectTypeName ) );
	}
	...
	//	LoadUri( Path.Combine( Application.streamingAssetsPath, "sample.html" ) );
		LoadUri( Path.Combine( Application.dataPath, "HTML/my_index.html" ) );
	}
